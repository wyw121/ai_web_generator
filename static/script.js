...
async function generateImage() {
    ...
    imageContainer.innerHTML = ''; // 清空当前的图像

    if (prompt) {
        // 显示用户的提示
        let userPromptDiv = document.createElement("div");
        userPromptDiv.textContent = `You: ${prompt}`;
        userPromptDiv.style.color = "white";
        imageContainer.appendChild(userPromptDiv);
...
async function generateImage() {
    const prompt = document.getElementById("input-prompt").value;
    const imageContainer = document.getElementById("image-container");
    // imageContainer.innerHTML = ''; // 如果需要每次清空当前图像，可以取消注释

    if (prompt) {
        // 显示用户的提示
        let userPromptDiv = document.createElement("div");
        userPromptDiv.textContent = `You: ${prompt}`;
        userPromptDiv.classList.add("message-container", "user-prompt");
        imageContainer.appendChild(userPromptDiv);
        ...
        aiResponseDiv.classList.add("message-container", "ai-response");
        imageContainer.appendChild(aiResponseDiv);

        // 显示AI的回复图像
        data.data.forEach(img => {
            let imgElement = document.createElement("img");
            imgElement.src = img.url;
            imgElement.classList.add("ai-image");
            aiResponseDiv.appendChild(imgElement); // 将图像添加到AI回复容器中
        });
